...
exports.generalDetails = function (config, repoName) {
    ...
    if (fileSystem.existsSync(config.dirname + "/" + config.repoDir + "/" + repoName + ".git/history.json")) {
        var historyString = fileSystem.readFileSync(config.dirname + "/" + config.repoDir + "/" + repoName + ".git/history.json");
        var historyJSON = {};
        var branchName = [];
        try {
